...
- [x] A2 — on the phone, swipe the installed app away from recent apps and
      reopen it. It opens on the tab bar (Read · Vocab · Review · Settings).

## Part B — paste, render, persist (FR-C1, C2, C8)

- [x] B1 — Read → paste a real passage of a few paragraphs (a WhatsApp
      ...
      other.
- [x] B2 — swipe the app away and reopen it: the same text is still there.

## Part C — speech (FR-C4, FR-I1)

- [x] C1 — Settings → Voice lists an Urdu voice first; **ur-PK** is selected
      (or pick it) and the sample plays in Urdu.
- [x] C2 — back on Read, tap single words: each is spoken in Urdu and starts
      feeling instant (the target is under 300 ms).
- [x] C3 — tap a second word while the first is still speaking: the first
      stops and the second plays. No queue builds up.

## Part D — selection, Speak, Define (FR-C3, C5, C7)

- [x] D1 — long-press a word and drag the handles across several words, even
...
- [x] D2 — **Speak** reads the whole selection.
- [x] D3 — select a word you know is in the vault (one of the 36 imported)
      → **Define**: it shows your saved Roman/English and mastery.
- [x] D4 — select a word that is not in the vault → **Define**: "Not in your
...
- [x] E1 — select a new word → **Add**. The Urdu field holds the word, the
      heading says WORD, and Notes holds the sentence it came from.
- [x] E2 — fill English with `smoke test` and **Save**. You see "ADDED …
      Saved as a word".
- [x] E3 — select the same word again → **Add** → **Save**: it says "Already
      in your vault" and shows the entry you just saved, rather than an error.
- [x] E4 — select two or more words → **Add**: the heading says PHRASE.
      **Cancel** without saving.
- [x] E5 — open `https://urdu.umber-amber.workers.dev/api/export` in the
...
- [x] All of A–E pass. Note anything that felt slow or awkward, even if it
