...
- [x] E1 — start a session, **Reveal**, turn on **airplane mode**, and tap a
      ...
      advances.
- [x] E2 — work through a whole session to the end (use Skip to go faster): the
      tally appears by itself after the last card.
- [x] E3 — if nothing is due, **Start review** is greyed out. You can't
...
- [x] F1 — **Review** shows a **Review ahead (days)** box set to 0. Set it to
...
- [x] F2 — **Vocab**: each row shows a coloured pill such as "2 • Basic",
      warm colours for low levels and teal for high ones. The item detail shows
      the same pill next to Mastery.
- [x] F3 — set **Sort** to Mastery, swipe the app away and reopen it: the Vocab
      list is still sorted by Mastery. Search and the other filters start empty.

- [x] F5 — the D2 colour bug is gone: **Reveal**, then look before touching
...
- [x] F4 — **English → Urdu** grading is gentler on misses. Note a word's
...
- [x] All green (2026-09-22). Notes: D2 colour bug fixed and confirmed in F5.
