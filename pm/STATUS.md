...
- f07 `coach-client` — 🟡 IN PROGRESS, s01 broker + s02 tool routes built, `pnpm check` green; s03 Voice screen next → `features/f07-coach-client.md`
...
4. **f07 `coach-client`** s03 Voice screen (WebRTC, reducer, tool relay, lifecycle), then s04 spend. Sponsor: put `OPENAI_API_KEY` in `.dev.vars` for the desktop run. Inbox empty. Toolchain: AVG Hardened Mode + CyberCapture off; never add exceptions.
