...
- **f03 `reader`** 🟡 in progress (s06 next, opened 2026-09-17) — paste-and-read Nastaliq, tap-to-speak, selection action bar, add and define (FR-C1..C8 + the FR-I1 voice picker) → `features/f03-reader.md`
...
2. **f03 `reader` is open; s01-s05 are built (2026-09-17).** Paste, Nastaliq render, tokens, tap-to-speak, the voice picker and the selection action bar pass locally; nothing has run on the phone yet. Next is s06, Add to vocab. A sponsor deploy first would check the s05 bar against the real Android selection toolbar. Sponsor still owes an answer on the s07 Define links (Rekhta, Wiktionary, Google Translate?). Slices and open questions are in `features/f03-reader.md`. `spikes/speech/` is deleted — its `speak()` now lives in `src/reader/speech.ts`.
