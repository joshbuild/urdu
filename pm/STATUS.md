...
- f11 `vocab-check` — 🟡 IN PROGRESS, s01 rotation built 2026-09-24 (migration 0004 local only), s02 corrections next → `features/f11-vocab-check.md`

## Next Session Pointers
*The 1–3 concrete next actions for a cold start.*

1. **f11 `vocab-check`**: s02 corrections (`parseCorrections`, `POST /api/handoffs/corrections` with `?preview=1`, field-guarded apply, guarded reset, stamping, row transition), then s03 client. The sponsor must apply 0004 remotely before the deploy that carries it.
