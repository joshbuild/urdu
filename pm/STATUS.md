...
2. **f03 `reader` is open; s01-s04 are built (2026-09-17).** Paste, Nastaliq render, tokens, tap-to-speak and the voice picker all work locally; nothing is on the phone yet. Next is s05, the selection action bar. Slices and open questions are in `features/f03-reader.md`. `spikes/speech/` is deleted — its `speak()` now lives in `src/reader/speech.ts`.
