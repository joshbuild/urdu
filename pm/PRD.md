...
- **FR-F9** *(planned, f11, 2026-09-23)* Accuracy check for existing items. The Vocab screen's **Copy check prompt** copies the 20 least recently checked items (id plus every present field; never-checked first) and asks for corrections only, each with a reason; **Paste corrections** previews each changed field as old → new and applies only the items the sponsor accepts, overwriting under the FR-D2 edit rules. The FR-F7 guards hold (echoed `urdu` must match, unknown ids reported, repeated `handoff_id` a no-op), and a field changed since the preview is not overwritten. A suggested Urdu spelling is shown as a flag, never applied. Applying stamps `checked_at` on every item in the batch, corrected or not. Scheduling is untouched unless the item's reset tick is set, which applies the FR-F8 reset.
