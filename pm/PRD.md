...
- **FR-F9** *(planned, f11, 2026-09-23)* Accuracy check for existing items. The Vocab screen's **Copy check prompt** copies up to 20 items (id plus every present field) and asks for corrections only, each with a reason; **Paste corrections** previews each changed field as old → new and applies only the items the sponsor accepts, overwriting under the FR-D2 edit rules. The FR-F7 guards hold (echoed `urdu` must match, unknown ids reported, repeated `handoff_id` a no-op), and a field changed since the preview is not overwritten. Scheduling is untouched unless an optional reset is chosen (open in f11).
