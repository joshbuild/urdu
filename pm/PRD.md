...
- **FR-B3** *(v1, with FR-G Option 1; amended 2026-09-18, DECISIONS 260918h)* External Coach clients call FR-F through `/coach/*` routes that accept only a separate bearer token from Worker secrets, accepted nowhere else; the PWA cookie is not accepted there. The v0 in-app voice Coach (FR-G Option 2) runs its tools through session-cookie routes over the same FR-F logic, because its tool calls arrive in the browser and the browser cannot hold a Coach token.
...
- **FR-C5** When any text is selected, an action bar appears with Speak, Add to vocab, Define, docked just above the tab bar (a bar floating above the selection collided with Android Chrome's own selection toolbar on the phone, 2026-09-17). No custom long-press handler; native selection handles are not overridden.
...
- **FR-F6** (added 2026-09-18, DECISIONS 260918f) The PWA has a **Copy prompt** button that copies a fixed, in-repo prompt for a ChatGPT web chat: the FR-F4 schema (proposals only), everyday Pakistani Urdu, practical Roman Urdu, concise English, and an instruction to reply with the JSON alone. The reply is pasted into the Paste-handoff screen; invalid JSON is rejected with the reason, never repaired.
- **FR-F7** (added 2026-09-18, DECISIONS 260918g) Fill-in round trip for incomplete items. The Vocab screen's **Copy fill-in prompt** copies up to 20 incomplete items (id plus every present field) and asks for the missing fields only; **Paste revisions** accepts `{handoff_id, revisions:[{vocab_id, urdu, roman?, english?, notes?, example_urdu?}]}`, previews each item's proposed fields, and on confirmation fills empty fields only. A row whose `urdu` differs from the stored item, or whose id is unknown, is rejected and reported. Scheduling is untouched; a repeated `handoff_id` is a no-op.
- **FR-F8** *(planned, f10, DECISIONS 260922a)* Coach vocab edit, a voice tool over the Vocab-tab edit rules: `{vocab_id?|urdu?, roman?, english?, notes?, tags?, reset_interval?}` overwrites the given fields on one item; `urdu` resolves by exact `urdu_key` match, and no match returns `not_found`, never a guess. The Coach reads the change back and waits for a spoken yes first. `reset_interval` (default false) sets the item to the first rung of the active ladder with `due_at` recomputed from the existing `last_reviewed_at` and no review event.
- **FR-F9** *(planned, f11, 2026-09-23)* Accuracy check for existing items. The Vocab screen's **Copy check prompt** copies the 20 least recently checked items (id plus every present field; never-checked first), which the Worker records under the prompt's `handoff_id`, and asks for corrections only, each with a reason; **Paste corrections** previews each changed field as old → new with its own tick (on by default) and overwrites only the ticked fields, under the FR-D2 edit rules. The FR-F7 guards hold (echoed `urdu` must match, unknown ids reported, repeated `handoff_id` a no-op), and a field changed since the preview is not overwritten. A suggested Urdu spelling is shown as a flag, never applied. Applying stamps `checked_at` on every item in the batch, corrected or not. Scheduling is untouched unless the item's reset tick is set, which applies the FR-F8 reset.
- **FR-F5** *(v1, DECISIONS 260918h)* An OpenAPI 3.1 description of FR-F1..F3 is maintained in-repo for use as the Custom GPT Action schema.
...
- **Option 2: In-app voice (v0).** A Voice screen in the PWA connecting over WebRTC to GPT-Live-1 through a Worker-brokered session (FR-B5), with the Coach system prompt maintained in-repo, and tools bound to FR-F1..F3 callable mid-conversation. Per-session and daily spend visible in Settings; a soft daily cap (default $0.50) warns and a hard cap (default $1.00) ends the live session client-side and makes the Worker refuse new sessions; both caps are editable in Settings (amended 2026-09-18, DECISIONS 260918h). *Planned, f10 (DECISIONS 260922a):* Urdu-script words in the transcript are tappable reader-style (FR-C speak and selection bar), and the most recent session's transcript is kept on the device until the next session starts (not cleared by Lock, not exported). Session history and transcript expiry are v1.
