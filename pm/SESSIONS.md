...
- 260917e · f02 airtable-import · Stages 2 and 3 shipped: `scripts/airtable-csv.ts` (RFC 4180 parse + Appendix C mapping, 25 unit tests in a new node `scripts` Vitest project) and `scripts/airtable-import.ts` (unlock, batch, `--dry-run`, markdown cross-check report, exit 0 only when empty); dry run over the real export clean at 36/0/0/0, live run against local D1 created 36 then updated 36 with 0 review_events — idempotence proven end to end; both f01 carry-forwards folded in, the secret scan now names-and-values on `dist/client` and values-only on `dist/urdu` with a planted-secret negative control; `pnpm check` green at 232 → `features/f02-airtable-import-journal.md`
